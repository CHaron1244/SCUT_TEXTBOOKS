...
//    	cout<<"A["<<i<<"]="<<A[i]<<endl;
    ...
    B[n].push_back(-n);//这里的n是输入的n加1了 
    B[n].push_back(n);
    int now = 0;// now 记录了双指针区间 [j, i] 中有几个完整的需求区间
    for (int i = 1, j = 1; i <= n; i++) {
        // 双指针右端点移动一步，增加右端点为 i 且位于 [j, i] 里的需求区间
        for (int x : B[i]) //跟i能组成区间的另一个端点 
			if (x > 0 && x >= j) //x是左端点,i是右端点   j   x   i 
				now++;//包含了[x,i]这个区间 
        // 求出 j = p_i + 1
        while (now > 0 && j <= i) {
            // 双指针左端点移动一步，减少左端点为 j 且位于 [j, i] 里的需求区间
            for (int x : B[j]) //跟j能组成区间的另一个端点 
				if (x < 0 && -x <= i) //x是右端点,i是左端点  j 	 x	 i		
					now--;
            j++;
        }
        assert(now == 0);
...
//    	cout<<"LIM["<<i<<"]="<<LIM[i]<<endl;
    ...
    for (int i = 2; i <= n; i++) {
        ...
        f[i] = f[q[head]] + A[i];
        while (head < tail && f[q[tail - 1]] >= f[i]) tail--;//保持增
        q[tail++] = i;
//        cout<<"端点"<<i<<"对应的lim值是:"<<lim<<endl;
    }
    printf("%lld\n", f[n]);
}
int main() {
